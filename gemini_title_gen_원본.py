...
# ======================= 세션 상태 초기화 =======================
for key in ["lang", "api_key", "generated_titles", "selected_titles"]:
    if key not in st.session_state:
        st.session_state[key] = "ko" if key == "lang" else ""

# ======================= 언어 선택 바 =======================
top1, top2 = st.columns([7, 1])
with top2:
    lang_choice = st.selectbox(" ", ["🇰🇷", "🇺🇸"], label_visibility="collapsed")
    st.session_state["lang"] = "ko" if "🇰🇷" in lang_choice else "en"
...
T = {
    ...
    "title_lang": "제목 생성 언어" if is_ko else "Title Generation Language",
    "rules": "1️⃣ 제목 생성 규칙",
    "word_limit": "최대 단어 수",
    "style": "제목 스타일",
    "guide": "제목 가이드라인",
...
st.session_state.api_key = st.sidebar.text_input(T["api_key"], type="password", value=st.session_state.api_key)
...
if st.button(T["generate"]):
    prompt = (
        ...
        f"Analyze the examples given below, and generate up to 20 high-quality paper titles in {title_lang_code}.\n"
    ...
    try:
        ...
        titles = [t.strip("•- ") for t in res.text.strip().split("\n") if t.strip()]
        st.session_state.generated_titles = titles

        st.markdown(f"### {T['select_titles']}")
        st.session_state.selected_titles = st.multiselect("👇 아래에서 원하는 제목을 선택하세요", titles)
...
if st.session_state.selected_titles and st.button(T["combine_titles"]):
    prompt = (
        f"You are an academic paper title combiner.\n"
        f"From the selected {len(st.session_state.selected_titles)} titles below, combine styles, patterns, and wording to generate 5 new and high-quality suggestions in {title_lang_code}.\n\n"
    )
    for t in st.session_state.selected_titles:
        prompt += f"- {t}\n"
    prompt += "\nPlease return exactly 5 distinct titles."
